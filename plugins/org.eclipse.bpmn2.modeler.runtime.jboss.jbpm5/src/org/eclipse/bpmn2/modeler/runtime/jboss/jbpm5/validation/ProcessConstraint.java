	...
	public class ProcessValidator extends AbstractBpmn2ElementValidator<Process> {
		...
		public IStatus validate(Process process) {
			...
			if (name==null || name.isEmpty()) {
				addStatus(process, "packageName", Status.ERROR, Messages.ProcessConstraint_No_Package_Name, process.getName(), process.getId());
			}
			else if (!JbpmModelUtil.isValidPackageName(name)) {
				addStatus(process, "packageName", Status.ERROR, "Package name is invalid: {0}", name);
			}
	
			name = process.getName();
			if (name==null || name.isEmpty()) {
				addStatus(process, "name", Status.ERROR, Messages.ProcessConstraint_No_Process_Name, process.getId());
			}
